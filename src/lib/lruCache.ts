...
export class LRUCache<T = unknown> {
  ...
  set(key: string, value: T, options: CacheOptions = {}): void {
    ...
    if (this.stats.totalSize + size > this.maxSize || this.cache.size >= this.maxEntries) {
      this.evictEntries(
        Math.ceil((this.stats.totalSize + size - this.maxSize * 0.8) / 1024), 
        3
      );
    }
  ...
  private evictEntries(sizeToFreeKB: number, minEntries: number): void {
    ...
    for (const priority of ['low', 'normal', 'high']) {
      ...
      for (let i = 0; i < this.lruOrder.length; i++) {
        ...
        if (!entry || priorityOrder[entry.priority as keyof typeof priorityOrder] > priorityOrder[priority as keyof typeof priorityOrder]) {
          continue;
        }
    ...
    if (entriesEvicted > 0) {
      logInfo(`LRU cache eviction: freed ${Math.round(bytesFreed/1024)}KB from ${entriesEvicted} entries`);
    }
  }
  
  /**
   * Estimate size of an object in bytes
   * This is a rough approximation as JS doesn't provide exact object sizing
   */
