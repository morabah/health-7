...
function compareValues(a: unknown, b: unknown, descending = false): number {
  ...
  if (typeof aComp === 'string' && typeof bComp === 'string') {
    return descending ? bComp.localeCompare(aComp) : aComp.localeCompare(bComp);
  }

  if (typeof aComp === 'number' && typeof bComp === 'number') {
    return descending ? bComp - aComp : aComp - bComp;
  }
 ...
 */
function setMemoryCacheData<T>(key: string, data: T, options: Partial<CacheOptions> = {}): void {
...
export async function getOptimizedDoctors(options: FilterOptions = {}): Promise<Doctor[]> {
  ...
  try {
    ...
    try {
      ...
      const result = await callApi<{ success: boolean; doctors: Doctor[] }>(
        'adminGetAllDoctors',
        adminCtx
      );
...
export async function getOptimizedAppointments(
  ...
  try {
    ...
    const response = await callApi<{ success: boolean; appointments: Appointment[] }>(
...
export async function getOptimizedNotifications(
  ...
  try {
    ...
    if (response && response.success) {
      ...
      notificationRequestTracker.backoffInterval = Math.max(
        0,
        notificationRequestTracker.backoffInterval - 500
      );
      ...
      notificationRequestTracker.backoffInterval = Math.min(
        30000,
        notificationRequestTracker.backoffInterval + 1000
      );
      notificationRequestTracker.consecutiveRequests++;
    }
    ...
    if (response && response.success && Array.isArray(response.notifications)) {
      ...
      // Also store in LRU cache for cross-component usage
      if (cacheKey) {
        enhancedCache?.set(CacheCategory.NOTIFICATIONS, cacheKey, processedData, {
          ttl: CACHE_TTL_CONFIG.notifications,
        });
      }
    ...
    if (cacheKey) {
      const staleData = enhancedCache?.get<Notification[]>(CacheCategory.NOTIFICATIONS, cacheKey, {
        forceRefresh: false,
      });
 ...
 */
function processNotificationsData(
...
export { setMemoryCacheData, getMemoryCacheData, clearMemoryCache, evictCacheEntries };
