...
// Define type guard for errors with a retryable property
interface ErrorWithRetryable extends Error {
  retryable: boolean;
}

function isErrorWithRetryable(error: unknown): error is ErrorWithRetryable {
  // Ensure it's an Error object and has the retryable boolean property
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return error instanceof Error && typeof (error as any).retryable === 'boolean';
}
...
export async function callApiWithErrorHandling<T>(
  ...
  while (true) {
    try {
      ...
      const shouldRetry =
        config.retry && retryCount < (config.maxRetries || 0) && isRetryableError(normalizedError);
...
export function isRetryableError(error: Error): boolean {
  // Use the type guard to safely access the retryable property
...
export async function parseApiError(response: Response): Promise<Error> {
  try {
    ...
    return new ApiResponseError(data.message || `API Error: ${response.status}`, {
      statusCode: response.status,
      response,
      data,
    ...
    return new ApiResponseError(`API Error: ${response.status}`, {
      statusCode: response.status,
      response,
      retryable: response.status >= 500 || response.status === 429,
    });
