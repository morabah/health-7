...
const NotificationBell: React.FC<NotificationBellProps> = ({ className }) => {
  ...
  const markAsRead = async (notificationId: string) => {
    ...
    try {
      await callApi('markNotificationAsRead', { uid: user.uid, role: user.role }, { notificationId });
      ...
      >
        ...
        {unreadCount > 0 && (
          <div className="absolute top-0 right-0 transform translate-x-1/4 -translate-y-1/4">
            <Badge variant="danger" size="xs" pill>{unreadCount > 9 ? '9+' : unreadCount}</Badge>
          </div>
        )}
      </button>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <Link 
                          ...
                          className="block text-sm font-medium text-slate-900 dark:text-white hover:text-primary dark:hover:text-primary-400"
                          onClick={() => {
