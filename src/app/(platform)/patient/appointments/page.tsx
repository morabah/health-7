...
const Row = ({ tab }: { tab: (typeof tabs)[number] }) => (
  <Card className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 p-4">
    <div>
      ...
      <p className="text-sm text-slate-500 dark:text-slate-400">Dermatology</p>
    ...
    <div className="mt-3 sm:mt-0 sm:ml-4 flex flex-col items-start sm:items-end gap-2">
      ...
      <div className="flex gap-2">
        <Button size="sm" variant="ghost" onClick={() => logInfo('View details')}>
          <User size={14} className="mr-1" />
          Details
        </Button>
        {tab === 'Upcoming' && (
          <>
            <Button size="sm" variant="secondary" onClick={() => logInfo('Reschedule')}>
              <Clock size={14} className="mr-1" />
              Reschedule
            </Button>
            <Button size="sm" variant="danger" onClick={() => logInfo('Cancel')}>
  ...
  useEffect(() => {
    ...
    try {
      logValidation(
        '3.8',
        'success',
        'Patient appointments page with tabs & placeholder rows implemented.'
      );
    } catch (e) {
