...
export default function DoctorVerificationPage() {
  ...
  const doctorsWithUserInfo = pendingDoctors.map((doctor: DoctorProfile) => {
    // Since we know all users should have an id, we can do a direct find with a type assertion
    const user = users.find(u => 'id' in u && u.id === doctor.userId);
    return {
      ...doctor,
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          ...
          {doctorsWithUserInfo.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-200">
              {doctorsWithUserInfo.map(
                (
                  doctor: DoctorProfile & { firstName: string; lastName: string; email: string }
                ) => (
