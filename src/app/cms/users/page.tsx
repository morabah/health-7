...
export default function UsersManagementPage() {
  ...
  useEffect(() => {
    ...
    if (usersData?.success && usersData.users) {
      // Assuming usersData.users from useAllUsers() actually provides UserProfile[] (with id)
      // despite its declared type in AdminGetAllUsersResponse potentially being just z.infer<typeof UserProfileSchema>[],
      // which is a common pattern if the id is merged in by the data fetching layer (e.g. getUsers).
      const userProfilesFromApi = usersData.users as UserProfile[];

      setUsers(
        userProfilesFromApi.map((user: UserProfile) => ({
          id: user.id,
          email: user.email ?? '',
          ...
          role: user.userType,
          status:
            (user as UserProfile & { status?: AccountStatus | 'PENDING' }).status ||
            (user.isActive ? AccountStatus.ACTIVE : AccountStatus.DEACTIVATED),
  ...
  const handleStatusChange = async (userId: string, newStatus: UserStatus) => {
    ...
    try {
      ...
      // Define a more specific type for the mutation result
      type MutationResult = {
        success: boolean;
        error?: string;
      ...
      const result = (await activateUserMutation.mutateAsync({
        ...
        status: apiStatus,
        reason:
          apiStatus === AccountStatus.SUSPENDED ? 'Account suspended by administrator' : undefined,
      })) as MutationResult; // Cast the result to the defined type

      if (!result.success) {
        throw new Error(
          result.error || 'Failed to update user status without specific error message'
        );
      }
  ...
  return (
    <div className="p-8">
      <header className="mb-8">
        ...
        <p className="text-gray-600">View and manage users, their roles, and account status</p>
      </header>
      ...
      {!isLoading && !activateUserMutation.isPending && (
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          ...
          {filteredUsers.length === 0 ? (
            <div className="p-8 text-center text-gray-500">No users match the current filters</div>
          ) : (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredUsers.map(user => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getRoleBadgeStyle(user.role)}`}
                        >
                          {user.role}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadgeStyle(user.status)}`}
                        >
                          {user.status}
                        </span>
                      </td>
